...
class UNet3D(Module):
    def __init__(self, n_in, n_out):
        ...
        self.ec0 = self.encoder_block(
            n_in, 1 * c, kernel_size=3, stride=1, padding=1)
        self.ec1 = self.encoder_block(
            c, c * 2, kernel_size=3, stride=1, padding=1)
        self.pool0 = nn.MaxPool3d(2)
        self.ec2 = self.encoder_block(
            c * 2, c * 2, kernel_size=3, stride=1, padding=1)
        self.ec3 = self.encoder_block(
            c * 2, c * 4, kernel_size=3, stride=1, padding=1)
        self.pool1 = nn.MaxPool3d(2)
        self.ec4 = self.encoder_block(
            c * 4, c * 4, kernel_size=3, stride=1, padding=1)
        self.ec5 = self.encoder_block(
            c * 4, c * 8, kernel_size=3, stride=1, padding=1)
        self.pool2 = nn.MaxPool3d(2)
        self.ec6 = self.encoder_block(
            c * 8, c * 8, kernel_size=3, stride=1, padding=1)
        self.ec7 = self.encoder_block(
            c * 8, c * 16, kernel_size=3, stride=1, padding=1)
        ...
        self.dc6 = self.decoder_block(
            c * 8, c * 8, kernel_size=2, stride=2, padding=0)
        ...
        self.dc3 = self.decoder_block(
            c * 4, c * 4, kernel_size=2, stride=2, padding=0)
        ...
        self.dc0 = self.decoder_block(
            c * 2, n_out, kernel_size=1, stride=1, padding=0)
        self.dl = nn.ConvTranspose3d(
            n_out, n_out, kernel_size=1, stride=1, padding=0)
...
class DeepN4:
    ...
    def fetch_default_weights(self):
        ...
        """
        fetch_model_weights_path = get_fnames(
            name="deepn4_default_torch_weights")
    ...
    def predict(self, img, img_affine, *, voxsize=(1, 1, 1)):
        ...
        (in_features, lx, lX, ly, lY, lz, lZ, rx, rX, ry, rY,
         rz, rZ, in_max) = (self.load_resample(resampled_T1))
